...
import { bar, paper, ink, muted, mono, sans, clashFab, spotifyDot, PERSON_COLORS, PERSON_INK, DAYS, VIEWS, shRow } from './theme.js';
  ...
  return (
    <div style={{ position: 'fixed', left: 0, right: 0, bottom: 0, zIndex: 50, pointerEvents: 'none' }}>
      ...
      <div style={{
        ...
        {/* Fan-out items */}
        {menuOpen && actions.map((a, i) => (
          <button key={a.key} onClick={a.onClick} className="fx-enter"
            style={{ animationDelay: `${i * 35}ms`, pointerEvents: 'auto', display: 'inline-flex', alignItems: 'center', flexDirection: side === 'left' ? 'row' : 'row-reverse', gap: 10, minHeight: 44, padding: side === 'left' ? '0 16px 0 6px' : '0 6px 0 16px', borderRadius: 26, border: 'none', backgroundColor: bar, color: ink, ...sans, fontSize: 14, fontWeight: 700, cursor: 'pointer', boxShadow: shRow }}>
        ...
        <button
          ...
          aria-haspopup="menu" aria-expanded={menuOpen} aria-label={menuOpen ? 'Close actions' : 'Open actions'}
          ...
          {!menuOpen && clashCount > 0 && (
            <span aria-hidden="true" style={{ position: 'absolute', top: -2, right: -2, minWidth: 20, height: 20, padding: '0 5px', borderRadius: 10, backgroundColor: clashFab, color: '#fff', ...mono, fontSize: 11, fontWeight: 700, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 0 0 2px ${bar}` }}>{clashCount}</span>
          )}
        </button>
      </div>
