...
export default function LineupTab() {
  ...
  // When searching or filtering to picks, force every matching section open
  // so results are never hidden inside a collapsed accordion.
  ...
  // ── Grouped + filtered view for the active day ──────────────
  const groups = useMemo(() => {
    const daySets = sets.filter(s => s.day === activeDay);
    const filtered = daySets.filter(s => {
  ...
  const dayCount = useMemo(
    () => sets.filter(s => s.day === activeDay).length,
    [activeDay]
  );
  const shownCount = groups.reduce((n, g) => n + g.sets.length, 0);
    ...
    <div style={{ ...sans }}>
        ...
        <div style={{ backgroundColor: tmrwBg, border: `1px dashed ${tmrwGold}`, borderRadius: 8, padding: '12px 16px', marginBottom: 20, display: 'flex', alignItems: 'flex-start', gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: bodyMuted, marginTop: 3, lineHeight: 1.4 }}>
              Tap an artist to add them to your picks. Times aren’t announced yet — clash detection switches on automatically once they drop.
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ position: 'relative', marginBottom: 10 }}>
        ...
        <input
          ...
          style={{
            width: '100%', boxSizing: 'border-box', padding: '11px 36px 11px 30px',
            borderRadius: 8, border: `1px solid ${rule}`, backgroundColor: '#fff',
            color: ink, fontSize: 15, ...sans, outline: 'none',
          }}
        />
        {search && (
      ...
      {/* Filter row: my-picks toggle + count + expand/collapse */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        <button onClick={() => setMyPicksOnly(o => !o)}
          style={{
            padding: '6px 12px', borderRadius: 999, cursor: 'pointer',
            border: `1.5px solid ${myPicksOnly ? PERSON_COLORS[activePerson] : rule}`,
            backgroundColor: myPicksOnly ? PERSON_COLORS[activePerson] : 'transparent',
            color: myPicksOnly ? ink : muted, ...mono, fontSize: 10, fontWeight: 700,
            letterSpacing: '0.1em', textTransform: 'uppercase',
          }}>
          ★ {activePerson}'s picks
        </button>
        <span style={{ ...mono, fontSize: 10, color: muted }}>
          {shownCount}{shownCount !== dayCount ? ` / ${dayCount}` : ''} artists · {groups.length} stage{groups.length === 1 ? '' : 's'}
        </span>
        {!forceOpen && (
          <button
            onClick={() => setOpenStages(openStages.size >= groups.length ? new Set() : new Set(STAGE_ORDER))}
            ...
            <section key={stage} style={{ borderRadius: 10, border: `1px solid ${rule}`, overflow: 'hidden', backgroundColor: paper }}>
              ...
              <button onClick={() => !forceOpen && toggleStage(stage)}
                style={{
                  ...
                  border: 'none', borderLeft: `5px solid ${color}`, background: 'none',
                }}>
                <span style={{ ...mono, fontSize: 12, fontWeight: 700, color: ink, letterSpacing: '0.04em', textTransform: 'uppercase', flex: 1 }}>
                  {stage}
                </span>
                {picked > 0 && (
                  <span style={{ ...mono, fontSize: 10, fontWeight: 700, color: ink, backgroundColor: PERSON_COLORS[activePerson], borderRadius: 999, padding: '2px 7px' }}>
                    ★ {picked}
                  </span>
                )}
              ...
              {open && (
                <div style={{ borderTop: `1px solid ${rule}` }}>
                  {stageSets.map(set => {
                    ...
                    const others = PEOPLE.filter(p => p !== activePerson && picks[set.id]?.[p]);
                    return (
                      <button key={set.id} onClick={() => togglePick(set.id, activePerson)}
                        style={{
                          width: '100%', textAlign: 'left', cursor: 'pointer',
                          display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10,
                          padding: '10px 14px', border: 'none',
                          borderTop: `1px solid ${rule}55`,
                          borderLeft: myPick ? `5px solid ${PERSON_COLORS[activePerson]}` : '5px solid transparent',
                        ...
                        <div style={{ minWidth: 0 }}>
                          ...
                          <div style={{ ...mono, fontSize: 10, color: myPick ? bodyMuted : muted, marginTop: 2 }}>
                            {timeLabel(set)}
                            {isClash && <span style={{ color: '#c94040', fontWeight: 700, marginLeft: 8 }}>⚡ CLASH</span>}
                          </div>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
                          ...
                          <span style={{
                            ...
                            backgroundColor: myPick ? PERSON_COLORS[activePerson] : 'transparent',
                          }}>
                            {myPick && (
