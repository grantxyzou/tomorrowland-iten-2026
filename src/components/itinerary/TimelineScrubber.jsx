...
export default function TimelineScrubber({ minute, isLive, onScrub, onSync }) {
  ...
  return (
    <div style={{ background: bar, padding: '14px 18px 12px' }}>
      {/* Header row: label + Sync */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <span style={{ ...mono, fontSize: 9, letterSpacing: '0.18em', color: caption }}>TIMELINE</span>
        <button onClick={onSync} aria-label="Sync to now"
          style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer', padding: 0, ...mono, fontSize: 9, letterSpacing: '0.14em', color: isLive ? tmrwGold : caption }}>
          <span style={{ width: 7, height: 7, borderRadius: '50%', background: isLive ? tmrwGold : 'transparent', border: isLive ? 'none' : `1.5px solid ${caption}`, boxShadow: isLive ? `0 0 8px ${tmrwGold}` : 'none' }} />
          {isLive ? 'LIVE' : 'SYNC'}
        </button>
      </div>
      ...
      <div
        ...
        <div ref={trackRef} style={{ position: 'relative', width: '100%', height: 6, borderRadius: 3, background: well, overflow: 'visible' }}>
          {/* Gold active windows: 00:00–02:00 and 14:00–24:00 (festival hours) */}
          <div style={{ position: 'absolute', top: 0, bottom: 0, left: 0, width: `${pct(120)}%`, background: tmrwGold, borderRadius: 3, opacity: 0.85 }} />
          <div style={{ position: 'absolute', top: 0, bottom: 0, left: `${pct(840)}%`, right: 0, background: `linear-gradient(to right, ${well}, ${tmrwGold} 12%)`, borderRadius: 3, opacity: 0.85 }} />
          {/* Thumb */}
          <div style={{ position: 'absolute', left, top: '50%', transform: `translate(-50%,-50%) scale(${pressing ? 1.12 : 1})`, width: 20, height: 20, borderRadius: '50%', background: '#fff', border: `2.5px solid ${tmrwGold}`, boxShadow: '0 2px 8px rgba(0,0,0,0.4)', transition: 'transform var(--dur-press) var(--ease-out)' }} />
        </div>
      </div>

      {/* Hour labels */}
      <div style={{ display: 'flex', justifyContent: 'space-between', ...mono, fontSize: 9, color: caption, marginTop: 8, letterSpacing: '0.06em' }}>
